...
function PRDSection({ data }: PRDSectionProps) {
  const [expandedStories, setExpandedStories] = useState<Set<string>>(new Set<string>());
  ...
  if (!data?.stories || data.stories.length === 0) {
    return (
      <div className="space-y-4">
        <h1 className="text-2xl font-semibold text-gray-900">Product Requirements (PRD)</h1>
        <p className="text-gray-500 text-sm">No user stories generated yet.</p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        {data.stories.map((story, index) => {
          const storyId = story.id || `US${index + 1}`;
                ...
                <div className="flex items-start gap-3 text-left flex-1">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="text-sm text-gray-600">
                      As a <strong>{story.persona || "User"}</strong>, I want to{" "}
                      {story.want || "..."} so that {story.so || "..."}
                    </div>
                  </div>
                </div>
